...
class GAuthenticator_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function config(Typecho_Widget_Helper_Form $form)
    {
		...
		$element = new Typecho_Widget_Helper_Form_Element_Text('SecretQRurl', NULL, '', _t('二维码的网址'), '复制打开就可以显示当前SecretKey的二维码,扫描就可以绑定');
		$form->addInput($element);
    ...
    public static function configHandle($config, $is_init)
    {
		if ($is_init) {//如果是第一次初始化插件
			...
			$config['SecretQRurl'] = 'http://qr.liantu.com/api.php?text='.urlencode('otpauth://totp/'.Helper::options()->title.'?secret='.$config['SecretKey']);//生成安全密钥的二维码网址
		}else{
